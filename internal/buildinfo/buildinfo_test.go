...
func TestVersionFallbacksAndInstallersStayAligned(t *testing.T) {
	...
	for _, check := range []struct{ path, marker string }{
		{"internal/site/src/lib/build-info.ts", `"0.2.6-dev"`},
		{"supplemental/scripts/install-agent.sh", `PRODUCT_VERSION="0.2.6"`},
		{"supplemental/scripts/install-hub.sh", `PRODUCT_VERSION="0.2.6"`},
	} {
		...
		if !strings.Contains(string(data), check.marker) {
			t.Errorf("%s is not aligned with v0.2.6", check.path)
		}
	}
}
